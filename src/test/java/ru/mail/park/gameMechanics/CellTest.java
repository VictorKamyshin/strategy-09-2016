...
public class CellTest {
    ...
    public void isCellRightCreated(){
        final BoardCell testCell = new BoardCell(0, null);
    ...
    public void coastCellTest(){
        ...
        for(int i = 0; i < mediumBoardSize; ++i) {
            for (int j = 0; j < mediumBoardSize; ++j) {
                if(i==0&&j==0){
                    boardMap[i][j] = new MockCell(0, new CoordPair(0,0), null);
                    ...
                    boardMap[i][j] = new BoardCell(mediumBoardSize * i + j, null);
        ...
        boardMap[targerCell.getX()][targerCell.getY()].moveIn(testPiratId, results, deadPirats);
        assert(deadPirats.isEmpty());
    ...
    public void piratMoveTest(){
        ...
        for(int i = 0; i < smallBoardSize; ++i) {
            for (int j = 0; j < smallBoardSize; ++j) {
                boardMap[i][j] = new BoardCell(smallBoardSize * i + j, null);
                boardMap[i][j].setNeighbors(new CoordPair(i,j));
            }
        }
        final Integer testPiratId = 0;
        ...
        final List<Integer> deadPirats = new ArrayList<>();
        boardMap[0][0].moveIn(testPiratId, results, deadPirats);
        assert(deadPirats.isEmpty());
        assert(results.size() == 1);
        assert(results.get(0).getStatus()>=0);
        for(int i = 0; i < smallBoardSize; ++i) {
